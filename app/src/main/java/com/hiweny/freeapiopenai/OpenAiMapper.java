...
/**
 * Map between OpenAI API format and the upstream free API.
 * Handles chat completions, text completions, model listing, and tool calls.
 */
public class OpenAiMapper {
    ...
    public static String models() {
        ...
        String[][] modelDefs = {
                {"free-api", "Free API (Gemini powered)"},
                {"gemini-pro", "Gemini Pro"},
                {"gemini-1.5-pro", "Gemini 1.5 Pro"},
                {"gemini-1.5-flash", "Gemini 1.5 Flash"},
                {"gemini-flash", "Gemini Flash"},
                {"gpt-4o", "GPT-4o (compatibility alias)"},
                {"gpt-4o-mini", "GPT-4o Mini (compatibility alias)"},
                {"deepseek-chat", "DeepSeek Chat (compatibility alias)"},
        };
    ...
    /**
     * Build a prompt string from OpenAI chat messages.
     * If tools are provided, injects tool definitions into the system message.
     */
    public static String promptFromChat(JSONObject req) {
        ...
        String toolPrompt = ToolCallParser.buildToolPrompt(tools);
        ...
        for (int i = 0; i < messages.length(); i++) {
            ...
            if ("tool".equals(role)) {
                sb.append(ToolCallParser.formatToolResult(msg));
                continue;
            }
                ...
                if (toolCalls != null && toolCalls.length() > 0) {
                    ...
                    for (int j = 0; j < toolCalls.length(); j++) {
                        ...
                        if (tc != null) {
                            ...
                            if (fn != null) {
                                sb.append("<tool_call>\n");
                                sb.append("{\"name\": \"").append(fn.optString("name", ""))
                                        .append("\", \"arguments\": ")
            ...
            if ("system".equals(role) && !toolPrompt.isEmpty()) {
                sb.append("system: ").append(content).append(toolPrompt).append("\n\n");
            } else {
    ...
    public static String chatCompletion(String model, String text, ToolCallParser.Result toolResult) {
        ...
        if (toolResult != null && toolResult.hasToolCalls) {
            // Content might be empty or contain text before tool calls
            if (!toolResult.content.isEmpty()) {
                put(msg, "content", toolResult.content);
            } else {
                put(msg, "content", JSONObject.NULL);
            }
            put(msg, "tool_calls", toolResult.toolCalls);
            put(choice, "finish_reason", "tool_calls");
        } else {
        ...
        put(root, "usage", usage(0, text != null ? text.length() : 0));
        return root.toString();
    }
    ...
    /**
     * Generate a streaming chunk with tool_calls delta.
     */
    public static String chatToolCallDelta(String model, JSONArray toolCalls) {
        JSONObject root = base("chat.completion.chunk", model);
        JSONArray choices = new JSONArray();
        JSONObject choice = new JSONObject();
        JSONObject delta = new JSONObject();
        put(delta, "tool_calls", toolCalls);
        put(choice, "index", 0);
        put(choice, "delta", delta);
        put(choice, "finish_reason", JSONObject.NULL);
        choices.put(choice);
    ...
    public static String chatDone(String model, String finishReason) {
        JSONObject root = base("chat.completion.chunk", model);
        JSONArray choices = new JSONArray();
        JSONObject choice = new JSONObject();
        put(choice, "index", 0);
        put(choice, "delta", new JSONObject());
        put(choice, "finish_reason", finishReason != null ? finishReason : "stop");
        choices.put(choice);
        put(root, "choices", choices);
        return root.toString();
    }
    ...
    public static String error(String type, String message, String code) {
        JSONObject root = new JSONObject();
        JSONObject error = new JSONObject();
        put(error, "message", message == null ? "unknown error" : message);
        put(error, "type", type);
